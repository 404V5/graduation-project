...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
	         <%
	             Object o=request.getAttribute("timuList");
	             ArrayList list=(ArrayList)o;
	             if(list.size()==0)
	             {
	                response.sendRedirect(path+"/atea/yuejuan/meiyoukaoshi.jsp");
	             }
	         %>
			<c:forEach items="${requestScope.timuList}"  varStatus="sta" var="timu">
				...
				<c:if test="${timu.timuLeixing=='jianda'}">
				    <table cellspacing="2" cellpadding="2" border="0">
						...
						<tr>
						   <td>
						        <input name="${timu.timuId}" type="text" size="60" value="${timu.timuDaanAsStu}"/>
						   </td>
						</tr>
					</table>
				</c:if>
			...
			<fieldset style="width:400px; margin-left:0px;"><legend class="fieldtitle">录入成绩</legend>
				<table class="bill" width="100%">
				    <form action="<%=path %>/fenshuAdd.action" name="" method="post">		
						<input type="hidden" name="shitiId" value="${requestScope.shitiId}"/>
						<input type="hidden" name="stuId" value="${requestScope.stuId}"/>
						输入分数：<input type="text" name="fenshu" size="10"/>
